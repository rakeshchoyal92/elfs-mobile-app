...
const RenderAQuestion = React.memo((compProps) => {
  const {
    question,
    index,
  ...
  const { responseType, questionText, isEndOfSurvey, isCondition } = question
  const props = {
    question,
    handleGetNextQuestion,
    initialValues,
  ...
  const renderValues = () => {
    if (isCondition && TESTING) {
      return (
        <View
          style={{ padding: 5, borderRadius: 10, backgroundColor: 'wheat' }}
        >
          <Text>Evaluate the condition {question.key} to</Text>
          ...
          <View span={12}>
            <Button
              ...
              onPress={() =>
                handleConditionResponse(
                  question.key,
                  question.conditionalNext.goToOnTrue
                )
              }
            >
              <Text>True</Text>
          ...
          <View span={12}>
            <Button
              ...
              onPress={() =>
                handleConditionResponse(
                  question.key,
                  question.conditionalNext.goToOnFalse
                )
              }
            >
              <Text>False</Text>
      ...
      // console.log(index, question.key)
      switch (responseType) {
...
const SurveyFill = (props) => {
  ...
  const handleTunnelRouting = (key, type) => {
    ...
    if (type === 'enter') {
      ...
      if (tunnelInfo.onEnd.startsWith('tunnel_')) {
  ...
  const evaluateCondition = (key) => {
    ...
    if (type === 'SINGLE') {
      ...
      if (conditionEvaluated) {
        ...
        } else if (goToOnTrue === 'return_to_tunnel') {
          return handleTunnelRouting(goToOnTrue, 'exit')
        } else if (goToOnTrue.startsWith('tunnel_')) {
          return handleTunnelRouting(goToOnTrue, 'enter')
        } else {
        ...
        if (nextQuestionKey.startsWith('c_')) {
          return evaluateCondition(nextQuestionKey)
        } else if (goToOnFalse === 'return_to_tunnel') {
          return handleTunnelRouting(goToOnFalse, 'exit')
        } else if (goToOnFalse.startsWith('tunnel_')) {
          return handleTunnelRouting(goToOnFalse, 'enter')
  ...
  const handleGetNextQuestion = async (question, valueSelected) => {
    ...
    if (valueSelected === '_VALUE_ANY' && question.responseType === 'LABEL') {
      valueSelected = true
    }
    ...
    if (valueSelected === '_SKIP_QUESTION') {
    ...
    if (valueSelected)
      nextQuestions.map((nq) => {
        ...
        if (nq.onValue === valueSelected) {
          ...
          let isReturnToTunnel = goTo === 'return_to_tunnel'
          if (isReturnToTunnel) {
            nextQuestionKey = tunnelInfo.onEnd
            let isNextTunnel = nextQuestionKey.startsWith('tunnel_')
            if (isNextTunnel) {
          ...
          } else if (goTo.startsWith('tunnel')) {
  ...
  const handleConditionResponse = (currentKey, goTo) => {
    if (goTo.startsWith('tunnel_')) {
      goTo = handleTunnelRouting(goTo, 'enter')
    } else if (goTo === 'return_to_tunnel') {
      goTo = handleTunnelRouting(goTo, 'exit')
    }
  ...
  return (
    <KeyboardAvoidingView
      ...
      style={{ flex: 1 }}
    >
      <AppLayout navigation={navigation} title="New Survey">
        {surveyQuestions && (
          <ScrollView
            ...
            {surveyQuestions &&
              surveyQuestions.map((question, index) => (
                <RenderAQuestion
                  ...
                  initialValues={initialValues}
                  setAResponse={setAResponse}
                  handleConditionResponse={handleConditionResponse}
